...
class Main(QWidget):
    def __init__(self, wav_file_list: list):
        ...
        window_size_layout, self.window_size_input = self.create_label_input_pair(
            "Window size", self.window_size
        )
        self.params_layout.addLayout(window_size_layout)

        window_overlap_layout, self.window_overlap_input = self.create_label_input_pair(
    ...
    def plot_wav(self, xmin, xmax):
        ...
        if len(peaks_selected) > 0:
            for i in np.arange(len(peaks_selected)):
                self.ax.plot(
                    [peaks_selected[i], peaks_selected[i]],
                    [0, np.max(rms_selected)],
                    "-g",
                    linewidth=1,
                )
        self.ax.plot()
    ...
    def delete_peaks(self, xmin, xmax):
        ...
        self.peaks_times = self.peaks_times[
            mask
        ]  # Mantiene solo i picchi fuori dall'intervallo
    ...
    def trova_picchi(self, xmin=0, xmax=0):
        ...
        if self.sender() is not None:  # 'Find peaks' button
            ...
            if xmin < 0:
                xmin = 0
            rms = self.rms[
                int(xmin * self.sampling_rate / self.overlap) : int(
                    xmax * self.sampling_rate / self.overlap
                )
            ]
        ...
        try:
            ...
            print(
                f"   - Distanza minima tra picchi: {min_distance_sec:.5f} sec ({min_distance_samples} campioni)"
            )
    ...
    def salva_canti(self):
        ...
        for i, peak_time in enumerate(self.peaks_times):
            ...
            if ini < 0 or fine > len(self.data):
                print(
                    f"Il picco {peak_time:.5f}s supera i limiti del file audio! Saltato."
                )
                continue
