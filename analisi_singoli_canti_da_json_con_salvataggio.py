...
class Main(QWidget):
    def __init__(self, wav_file_list: list):
        ...
        self.init_values()
        self.rows = []          # lista di dict
        self.df_results = None  # DataFrame finale (lo costruisci alla fine)
    ...
    def load_job(self, index: int):
        ...
        # 2) carica WAV (resetta strutture coerentemente col file)
        self.load_wav(self.wav_file)
        self.plot_wav(self.xmin, self.xmax)
    ...
    def load_wav(self, wav_file):
        ...
        # aggiugno un 0.05 di silenzio prima e dopo il canto
        if add_noise == True:
            new_data = add_noise_padding(data, self.sampling_rate, duration=0.025, noise_db=-20)
        else:
            new_data = data
        self.data = new_data
        self.xmin = 0
    ...
    def trova_picchi(self):
        ...
        try:
            ...
            if peaks.size == 0:
                # nessun picco trovato: pulisci e non andare avanti
                self.peaks_times = np.array([])
                self.peaks_int = np.array([])
                # opzionale: azzera canto / durata
                self.canto = np.zeros(len(self.rms) * self.overlap)
                self.plot_wav(self.xmin, self.xmax)
                return
    ...
    def trova_ini_fin(self):
        ...
        if p1 == p0: # un solo picco
            mask = self.rms[p0:] <= self.min_amplitude
            idx = np.where(mask)[0]
            if len(idx) > 0:
                p1 = p0 + idx[0]
            else:
    ...
    def save_results_clicked(self):
        ...
        self.envelope20 = np.mean(self.rms_canto >= soglia20)
        self.envelope50 = np.mean(self.rms_canto >= soglia50)
        self.envelope80 = np.mean(self.rms_canto >= soglia80)
        ...
        row = {
                    ...
                    "data":file_name.split("_")[1], 
                    "id":  file_name.split("_")[2], 
                    "note_peak":  sample/self.sampling_rate, 
                    "inter_note": None   , 
                    "bout":   None , 
                    "note_period":  None  , 
                    "durata_nota": self.durata_canto, 
                    "envelope20": self.envelope20, 
                    "envelope50": self.envelope50, 
                    "envelope80": self.envelope80, 
                   "fase_ascendente": self.fase_ascendente, 
                   "fase_discendente": self.fase_discendente, 
                   "pulse_number": len(self.peaks_times), 
                   "pulse_rate":  (len(self.peaks_times) - 1) / self.durata_canto  , 
        ...
        if len(self.rows) > 1:
            ...
            for r in self.rows:
                #print(r["note_peak"])
                if r["note_peak"] != row["note_peak"]:
                    nuove_rows.append(r)
            self.rows = nuove_rows
...
def add_noise_padding(data, sr, duration=0.1, noise_db=-40):
    """
    Aggiunge padding con rumore bianco prima e dopo il segnale.

    Parameters
    ----------
    data : np.ndarray
        Segnale audio (mono o stereo)
    sr : int
        Sampling rate
    duration : float
        Durata padding in secondi (default 0.1 s)
    noise_db : float
        Livello del rumore in dB rispetto al RMS del segnale
        (es. -40 significa 40 dB sotto il segnale)

    Returns
    -------
    new_data : np.ndarray
        Segnale con padding rumoroso
    """
    ...
    if signal_rms == 0:
        signal_rms = 1e-12  # evita divisioni per zero

    # Calcola RMS del rumore in base ai dB richiesti
    noise_rms = signal_rms * 10**(noise_db / 20)

    # Genera rumore normalizzato
    def generate_noise(shape):
        noise = np.random.randn(*shape)
        noise = noise / np.sqrt(np.mean(noise**2))  # RMS = 1
    ...
    # Se int16 → converti temporaneamente in float
    ...
    # Riporta al dtype originale se necessario
    if np.issubdtype(original_dtype, np.integer):
...
class ControlPanel(QWidget):
    def __init__(self, main):
        ...
        self.min_distance_input.setMaximum(0.3)  # Set maximum value
        ...
        self.max_distance_input.setMaximum(2)  # Set maximum value
        ...
        self.prominence_input.setMaximum(0.9)  # Set maximum value
    ...
    def leggi_json(self):
        ...
        for wav_key, wav_block in parameters.items():
            ...
            if "peaks_int_level1" in wav_block:
                continue
            print(f"SONO QUI E DEVO LEGGERE IL WAV LUNGO")
